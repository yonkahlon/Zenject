...
namespace Zenject
{
    public class GetFromPrefabComponentProvider : IProvider
    {
        ...
        public IEnumerable<ZenjectException> Validate(InjectContext context, List<Type> args)
        {
            if (_isValidating)
            {
                // This is necessary otherwise validation fails in some cases with
                // circular dependencies that are valid
                yield break;
            }

            _isValidating = true;

            foreach (var error in _prefabInstantiator.Validate(args))
            {
                yield return error;
            }

            if (_prefabInstantiator.GetPrefab().GetComponentsInChildren(_componentType).Length == 0)
            {
                yield return new ZenjectException(
                    "Expected to find at least one component with type '{0}' on prefab with name '{1}'",
                    _componentType, _prefabInstantiator.GetPrefab().name);
            }
