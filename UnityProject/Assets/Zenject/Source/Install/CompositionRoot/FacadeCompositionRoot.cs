            ...
            InstallerExtraArgs installerExtraArgs)
        {
            ...
            if (_facade == null)
            {
                throw Assert.CreateException(
                    "Facade property is not set in FacadeCompositionRoot '{0}'", this.gameObject.name);
            }

            if (!UnityUtil.GetParentsAndSelf(_facade.transform).Contains(this.transform))
            {
                throw Assert.CreateException(
                    "The given Facade must exist on the same game object as the FacadeCompositionRoot '{0}' or a descendant!", this.name);
            }
