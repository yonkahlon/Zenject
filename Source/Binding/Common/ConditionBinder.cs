using System;
using System.Linq;
using ModestTree;

namespace Zenject
{
    public class ConditionBinder : InheritInSubContainersBinder
    {
        public ConditionBinder(StandardBindingDescriptor binding)
            : base(binding)
        {
        }

        public InheritInSubContainersBinder When(BindingCondition condition)
        {
            Binding.Condition = condition;
            return this;
        }

        public InheritInSubContainersBinder WhenInjectedIntoInstance(object instance)
        {
            Binding.Condition = r => ReferenceEquals(r.ObjectInstance, instance);
            return this;
        }

        public InheritInSubContainersBinder WhenInjectedInto(params Type[] targets)
        {
            Binding.Condition = r => targets.Where(x => r.ObjectType != null && r.ObjectType.DerivesFromOrEqual(x)).Any();
            return this;
        }

        public InheritInSubContainersBinder WhenInjectedInto<T>()
        {
            Binding.Condition = r => r.ObjectType != null && r.ObjectType.DerivesFromOrEqual(typeof(T));
            return this;
        }

        public InheritInSubContainersBinder WhenNotInjectedInto<T>()
        {
            Binding.Condition = r => r.ObjectType == null || !r.ObjectType.DerivesFromOrEqual(typeof(T));
            return this;
        }
    }
}
